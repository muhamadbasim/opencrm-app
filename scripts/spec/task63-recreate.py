#!/usr/bin/env python3
"""
Task 6.3 — Recreate backend critical files from BACKEND-SOURCE-MANIFEST.json.

Iterates ALL 256 entries of BACKEND-SOURCE-MANIFEST.json. For each entry,
extracts the canonical content from the corresponding `backend/reference/files/*.md`
...
Reuses the extraction model from task62-recreate.py (and task61-recreate.py before that).
...
BACKEND_BASE = SKILL  # manifest entries store reference paths relative to the skill root

# Files already covered + verified by Task 6.1 (backend apps config files).
# Per task 6.3 description: "Skip files already covered by Task 6.1
# (apps/backend/{package.json, tsconfig.json, prisma.config.ts, Dockerfile})".
# These are still verified in PHASE 5 (post-write SHA), so the parity guarantee holds for
# the entire 256-entry manifest, but they are not rewritten.
TASK61_COVERED = {
...
def main(argv):
    ...
    print('\n=== PHASE 5: Post-write SHA verification (all 256 entries) ===')
    pass_count = 0
    fail_list = []
    for e in entries:
        target_path = os.path.join(OPENCRM_APP, e['target'])
    ...
    if fail_list:
        ...
        for t, msg in fail_list:
            print(f'    {t}: {msg}')
        return 4
    print('  OK: all 256 entries match expected SHA after write')
    return 0
